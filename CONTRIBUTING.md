...
This repo packages [RoboSats](https://github.com/RoboSats/robosats) for StartOS.
...
RoboSats ships the upstream `recksato/robosats-client` image directly — the package does not build its own container. Note that upstream tags carry an `-alpha` suffix.

1. Bump `dockerTag` in `startos/manifest/index.ts` to `recksato/robosats-client:v<new version>-alpha`.
